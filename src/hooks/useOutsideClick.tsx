...
const useOutsideClick = (
	...
	innerClickElemRefs: React.RefObject<HTMLElement>[] | undefined,
	onOutsideClick: () => void
) => {
	useEffect(() => {
		const handleClickOutside = (event: MouseEvent) => {
			const clickedAnInnerElemRef = innerClickElemRefs?.find((innerClickElemRef: React.RefObject<HTMLElement>) =>
				innerClickElemRef?.current?.contains(event.target as Node)
			);
